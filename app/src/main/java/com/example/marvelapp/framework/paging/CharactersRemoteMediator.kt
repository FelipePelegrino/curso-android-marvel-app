    ...
    override suspend fun load(
        ...
        return try {
            val offset = when (loadType) {
                // requisição pela primeira vez, ou usuário fez a ação de swip to refresh para carregar os dados
                LoadType.REFRESH -> 0
            ...
            database.withTransaction {
                ...
                remoteKeyDao.insertOrReplace(
                    RemoteKey(
                        nextOffset = responseOffset.plus(state.config.pageSize)
                    )
                )

                val charactersEntity = characterPaging.characters.map {
                    CharacterEntity(
                        id = it.id,
                        name = it.name,
            ...
            MediatorResult.Success(
                endOfPaginationReached = responseOffset >= totalCharacters
            )
        } catch (e: IOException) {
